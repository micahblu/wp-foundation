...
function sh_add_js() {
	if ( ! is_admin() ) {
	    ...
	    wp_enqueue_script( 'prism_js', get_stylesheet_directory_uri() . '/includes/prism/js/sh.js', '', '', true );
			wp_enqueue_style( 'prism_css',  get_stylesheet_directory_uri() . '/includes/prism/css/sh.css');
		}
	}
}
...
function sh_add_quicktags( $hook ) {
    if( 'post.php' == $hook ||  'post-new.php' == $hook )
		wp_enqueue_script( 'sh_quicktag_js', get_stylesheet_directory_uri(( 'js/quicktag.js', __FILE__ ), array( 'quicktags' ), '', true );
}